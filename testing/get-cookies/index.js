...
;(async () => {
  ...
  page.on("request", async (request) => {
    if (
      ["stylesheet", "fetch", "image", "media", "font"].includes(
  ...
  await page.evaluate(() => {
    ...
    document
      .getElementById("loginRedirectProviderList")
  ...
  while (!frame)
    frame = (await page.frames()).find((f) =>
      f.url().includes(".duosecurity.com/")
    )
  ...
  await frame.evaluate((OTP) => {
    ...
    ;[...document.getElementsByTagName("button")]
      .filter((x) => x.innerText.includes("Enter a Passcode"))[0]
      .click()
    // Input OTP
    ;[...document.getElementsByTagName("input")].filter(
      (x) => x.name === "passcode" && x.type === "text"
    )[0].value = OTP
    // Submit
    ;[...document.getElementsByTagName("button")]
      .filter((x) => x.innerText.includes("Log In"))[0]
      .click()
  }, OTP)
  ...
  await page.waitForFunction(
    `window.location.href.startsWith('https://lms.rpi.edu/')`
  )
  ...
  const response = await fetch(
    "https://lms.rpi.edu/learn/api/public/v3/courses",
    {
      headers: {
